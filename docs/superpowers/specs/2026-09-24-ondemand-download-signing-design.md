...
- ~~**升级前已删用户的目录仍可被认领**~~（#2 的覆盖面边界，第六轮 code-review 第 1 条）：
  **已在 §0.12 修复**（哨兵硬闸 + 扫盘补墓碑）。
  原记：见上方「#2 的覆盖面边界」。修法需**改判据**（`user_id = 0` 哨兵无条件阻断），
  不是补一处写入点 —— 需先裁定。
- ~~**记「释放」用的求值函数与读路径不一致**~~（第六轮 code-review 第 2 条）：
  **已在 §0.12 修复**（越界退化同步进 `_dir_name_of`）。
  原记：`auth._dir_name_of` = `display_name or username`（空则 `user_<id>`）；
  `main._scrape_dn_for` **多一步**「推导结果越出爬取根 ⇒ 退化为 `user_<id>`」。对**越界**的
  存量 `display_name`（如 `..\..\_x`，写入关口加固前落库的值），两者给出**不同**的名字：
  真实目录是 `user_<id>`，而改名时记下的释放名是 `..\..\_x`。
...
### 0.12 第七轮：第六轮 code-review 两条发现的裁定与落地（2026-09-24 同日）

第六轮审查提出两条「先于本轮存在」的缺陷（上一节末尾两条「仍未做」），用户逐条裁定：
**#1 改判据（哨兵无条件阻断）**、**#2 同步越界退化进 `_dir_name_of`**。两条均已实现。

#### 裁定与实现

| # | 裁定 | 实现 |
|---|---|---|
| #1 | **哨兵无条件阻断**（而非「参与序号比较」） | `auth._dn_released_keys` 引入 `blocked` 集合：`user_id == _DN_SENTINEL_UID` 的行**既不进 `mine` 也不进 `others`**，其 `dn` 键直接进 `blocked`，**无条件**从结果里剔除。新增 `auth._DN_SENTINEL_UID = 0` 常量（表**无外键**，故哨兵取值由代码约定；`users.id` 是 AUTOINCREMENT 从 1 起，0 安全） |
| #1（补） | 同时补上**当时根本没有哨兵行**的那一档 | 新增 `database._tombstone_orphan_scrape_dirs(conn)`：扫爬取根下**不属于任何存活用户**的目录名，各补一行哨兵墓碑。接入 `_migrate_if_needed` 作第 6 步，**独立标记** `tombstoned_orphan_scrape_dirs` |
| #2 | **同步越界退化进 `_dir_name_of`** | `auth` 新增 `_dn_within_root(name)`（本地实现 `main._is_within` 同一套 realpath 判据，**不能** import main —— 循环依赖）；`_dir_name_of` 推导结果越界时同样退化为 `user_<id>`，与 `main._scrape_dn_for` 对齐。`directory_name_error` 里的候选名 `eff` 同口径退化 |

**#1 为什么必须改判据，而不是「插一个很大的 id」**：哨兵若参与 `seq` 比较，它在写入那一刻
拿到的是**当时**的最大 id，此后任何真实用户再释放一次同名（新 id 更大）就**赢过它** ⇒
阻断静默失效。这与「哨兵 id 取多大」无关，是语义问题：哨兵表达的是「**这个名字的归属
事后无法判定**」，不是「某人最后释放过」。故必须是**硬闸**。

**#1（补）为什么还要扫盘**：硬闸解决的是「哨兵被顶掉」，而**升级前已删用户**那一档连哨兵
行都没有（无 `users` 行可推、也无墓碑写入点）—— 判据里等同「无人用过」。补扫一次盘即可。

**为什么扫盘必须一次性**：之后再无从分辨「无主的目录」与「当前用户还没爬过的目录」——
未来的删除都会自己写墓碑。故用**独立标记**只做一次，而不是每次启动都扫。
独立于 `migrated_scrape_dn_history`：两者失败重试互不牵连（扫盘失败**不打标记**、
打印后 rollback，下次重试）。

**⚠️ 扫盘的口径必须与认领判据同一个函数**（`auth._dir_name_of` + `auth._dn_key`）：
口径若不一致（漏了越界退化、或漏了大小写归一），就会把**自己人的**目录误判成无主、
把他的名字永久封掉 —— 这是本函数**唯一**的危险方向。故直接复用而不另写一份。
`ai` 子目录与根下的普通文件跳过（`main.scrape_packages` 同样跳过 `ai`）。

**#2 的连带收口**：越界存量用户与「名字恰好是 `user_<id>`」的新用户在**真实磁盘上共用
同一目录**，而 `directory_name_error` 的判重此前看不见（它用 `_dir_name_of`）。退化后
这条漏判重一并堵上。

#### live 暴露面（实测量化）

- 越界 / 含分隔符的存量名：**0 行**
- `scrape_dn_history` 升级时：**0 行**
- 爬取根下的孤儿目录：仅 `alice` / `alice2`，且因判据 3（目录已存在）本来就挡住别人取得
  该名 ⇒ 该档在 live 上**结构性不可达**（改判据是防止将来换库/换数据后可达）

#### 对 live 库的实际影响（如实记录）

本函数上线后第一次 `get_db()` 会对 `temp/app.db` 补 2 行哨兵（`alice` / `alice2`）并置上
标记。已在本次开发中实际发生（我用 `_tombstone_orphan_scrape_dirs` 的独立调用触发过一次），
核对结果：**33 名存活用户中无一人的目录名被误封**。这是**设计中的生产行为**，
重启 Flask 后同样会发生。
...
| 文件 | 结果 |
|---|---|
| `py/tests/test_scrape_ownership.py` | **84 passed**（81 + 3 新增） |
| `py/tests/test_scrape_dn_history_migration.py` | **8 passed**（4 + 4 新增） |

**新增用例（每条主腿都配对照腿）**：

| 用例 | 钉住什么 |
|---|---|
| `TestSentinelIsAHardGate::test_sentinel_blocks_even_after_a_later_release` | #1：哨兵行**在前**（序号小）、我的同名释放行**在后**（序号大）⇒ 该名仍不进我的 keys；对照腿：同批写入里一个**没有**哨兵的 `_sg_ok` 照常判给我（防「一刀切全冻住」）。⚠️ 反向写法（哨兵在后）测不出问题 —— 那时按序号比较也恰好是拒，是一条**永远绿的假腿** |
| `TestOutOfRootLegacyNameMatchesReadPath::test_dir_name_of_degenerates_like_scrape_dn_for` | #2 纯函数腿：`..\..\_x` / `../_x` / `D:/abs/_x` 三种越界形态下，`auth._dir_name_of` 与 `main._scrape_dn_for` **必须同值**且均为 `user_<id>` |
| `…::test_legacy_out_of_root_name_release_records_real_directory` | #2 **真实形态腿**：`_seed_raw_user` 直接 INSERT 一个越界存量名（绕过关口，这正是 L-7 豁免放过的形态），改名离开后释放行记的必须是 `user_<id>`、**不得**是越界串（否则当事人永远取不回自己的产物） |
| `TestOrphanScrapeDirsAreTombstoned::test_orphan_dir_gets_sentinel_and_cannot_be_claimed` | #1 补扫主腿；**前置断言**「扫描前该名确实判给了 V」—— 否则「拒」可能来自别的原因，断言成假绿 |
| `…::test_live_users_directory_is_not_tombstoned` | **对照腿**：活人自己的目录**一个哨兵都不补**（防「根下每个目录都补」把所有人封死） |
| `…::test_scan_is_one_shot` | 独立标记必须写上 + 二次调用不重复插 |
| `…::test_scan_is_wired_into_migration` | **接线腿**：前三/四条都是**直调函数** —— 删掉 `_migrate_if_needed` 里那一行调用它们照样全绿，而线上再也不会补墓碑 |

#### 变异验证（m17–m23，7 条，跑完均已还原，`grep -c MUTATION py/{auth,database,main}.py` 全为 0）

| 变异 | 结果 |
|---|---|
| m17 哨兵退回「按序号参与比较」 | **恰好 1 红**：哨兵硬闸用例 |
| m18 `_dir_name_of` 去掉越界退化 | **恰好 2 红**：#2 的两条 |
| m19 扫盘函数体直接 `return` | **3 红 1 绿**（对照腿「活人目录不补哨兵」保持绿 —— 它测的是「补多了」，测不出「什么都没补」，属预期） |
| m20 不把扫盘挂进 `_migrate_if_needed` | **恰好 1 红**：接线腿（其余 3 条绿 ⇒ 接线腿不空） |
| m21 扫盘不看「有主」 | **恰好 1 红**：对照腿 |
| m22 扫盘不写一次性标记 | **恰好 1 红**：幂等腿 |
| m23 根不存在不再早退 + 退回「读不到当空目录」 | **恰好 1 红**（收口第 2 条的用例） |

**m17 的覆盖面记录（如实）**：`test_ambiguous_name_from_old_format_is_given_to_nobody`
（迁移歧义哨兵）在 m17 下**仍绿** —— 迁移是先插用户行、后插哨兵，哨兵拿到的是最大 id，
此时「按序号比较」与「硬闸」行为一致，该用例**区分不了**两者。区分力只存在于
`TestSentinelIsAHardGate`（哨兵在前、真实用户在后）。这不是缺口，但值得记下来：
**只有当哨兵不是最后写入者时，「硬闸」才与「大序号」可区分**。

#### 第六轮 code-review 的收口（两条发现）

**收口 1 —— ⚠️ 两裁定的相互作用（发现属实，但修法被否，代价改为「记录 + 量测」）**

审查者指出：扫盘只看「存活用户**当前**的目录名」，于是**存活用户改名后遗留的旧目录**
（`scrape_dn_history` 里已有该用户的释放行）会被判为无主、补上哨兵 —— 而哨兵是无条件
硬闸 ⇒ 该用户从此**改不回原名**、旧目录里的产物在盘上却取不回。已用运行时探针复现
（扫描前 `directory_name_error` 返回 `None`，扫描后返回「该目录已被占用」）。
审查者给的修法方向是：**扫描前剔除 `scrape_dn_history` 里已有任意 `user_id` 记录的名字**。

**该修法被否，理由（决定性）**：它会让本修复**退化成空操作**。攻击者要拿到那个名字，
前提**正是**他**也**有一行同名释放记录 —— `auth.directory_name_error` 的判据 3 是
...
`own_keys` = 自己的**当前**目录名 + username 派生名；要越过它，攻击者只能靠 `hist_keys`
（自己的释放行）。**凡可被利用的名字，必然出现在 `scrape_dn_history` 里** ⇒ 按该修法
全部被排除在扫描之外 ⇒ 缺口原样复活，只是把「#2 的覆盖面边界」又写回了一遍。

**两档在数据上不可区分**：`(攻击者有过该名)` 与 `(我自己的旧目录)` 在表上长得一模一样
—— 被删用户升级**前**没有行，所以「只有我一行」既是清白证据也是攻击者形态。按本项目
一贯口径（**误放 > 误拒**）取「封」。

**代价的实际规模（实测量化）**：扫盘**只跑一次**，故只影响「本次上线**之前**就已改名、
且想改回去」的用户；live 库扫描时 `scrape_dn_history` 为 **0 行**（实测），即当前
**0 人**受影响。已在 `test_orphan_dir_gets_sentinel_and_cannot_be_claimed` 的 docstring 里
把这个代价与「为什么不能按审查者的方向修」一并钉住，防止将来被人「顺手修正」回空操作。

**收口 2 —— 已修**：根不存在被降级成「根是空的」，随后仍无条件写一次性标记
（原 `except OSError: entries = []`）。这与函数自称的「不打标记，下次重试」相矛盾，
且升级时「先起服务、后拷爬取目录」会把**唯一**一次机会永久用掉。
修法：`os.path.isdir(root)` 为假即早退、**不打标记也不打印**（真·全新部署下根会长期不存在，
而 `_migrate_if_needed` 每个请求都跑，打印会淹掉输出）；根存在却**读不出来**（权限等）
走外层 `except` ⇒ 打印 + 不打标记 + 重试。新增用例 + 变异 m23 钉住。

#### 自查

- **对照腿必须能测出「另一侧」**：m19 里「活人目录不补哨兵」保持绿是**对的** —— 那条腿的
  职责是防「补多了」，不是防「没补」。把它写成也能测「没补」是职责重叠，会让 m20/m21 的
  区分力消失。
- **接线腿不可省**：本轮新增的 4 条扫盘用例里，3 条直调函数 ⇒ 单靠它们，删掉调用点
  是**静默失效**。这正是「测试绿 ≠ 功能接上」的又一例。
- **审查者的修法方向也要自己验**：收口 1 的发现属实、但建议的修法会让保护失效 ——
  「缺陷描述对」不等于「修法对」，两者要分开核。

#### 验收

- `py/tests/test_scrape_ownership.py`：**84 passed**
- `py/tests/test_scrape_dn_history_migration.py`：**9 passed**（8 + 收口 2 的 1 条）
- 两文件合计：**93 passed**
- 全量：**877 passed / 0 failed**
  —— ⚠️ 同前，该数是**工作区合并态**，含**并行会话**在途用例，不得当成本任务增量。
  本任务自身增量就是上面两行（84 + 9）。
- `git diff` 口径：本轮只动 `py/auth.py`、`py/database.py`、两个测试文件
  （`py/main.py` **本任务无改动** —— 其唯一 hunk 已在 `39b8a9b` 提交）

#### 仍未做（本节更新）

- ~~**升级前已删用户的目录仍可被认领**~~ —— **已修**（本轮：哨兵硬闸 + 扫盘补墓碑）
- ~~**记「释放」用的求值函数与读路径不一致**~~ —— **已修**（本轮：越界退化同步进
  `_dir_name_of`，含漏判重的连带收口）
- ~~**上级改动引入的既有代价：扫盘封掉了「上线前已改名者的旧目录认领路」**~~
  —— **已修**（§0.13：认领判据加时间维度，扫盘退役；用户 2026-09-25 裁定「换判据」）
- 以下仍在（与上一节同一清单）：**删用户不清其爬取目录**（磁盘无界增长）、
  **迁移竞态**（`_ensure_columns` 无锁）、**测试隔离到 tmp**、**存量非法名治理**
  （in-root 分隔符，live 0 行）、**`temp/` 清理与 `temp/_.*` gitignore**
- **新记（本轮发现，未修）**：`scrape_dn_history` **无唯一约束**，`_tombstone_orphan_scrape_dirs`
  若被重复触发（标记被清、或失败重试）会对同一目录名补**多行**哨兵。当前无实际影响
  （哨兵进的是集合，重复行不改变判据），但表会无界增长。

### 0.13 第八轮：认领判据加时间维度（收口 1 的代价回收，2026-09-25）

用户裁定（原话）：**「换判据：加时间维度」** —— 兑现 §0.12 「仍未做」里的第一条待裁定项：
「扫盘封掉了『上线前已改名者的旧目录认领路』（实测 live **0 人**受影响）。若要恢复这条路，
得找一个能在数据上区分『我的旧目录』与『被删用户的目录』的信号 —— 当前表结构里没有」。

**找到的信号是时间**：目录有创建时刻（`os.stat(dir).st_ctime`），释放记录有写入时刻
（`scrape_dn_history.created_at`）。接上它，两档立刻可分。

#### 根因（收口 1 的成因，一句话）

扫盘判「无主」只用了一个维度 —— **这个名字对不对得上某个存活用户的当前目录名**
（`database.py` 的 `live` 集合），**缺了时间维度** ⇒「我自己的旧目录」与「被删用户的
目录」在数据上完全同形（§0.12 收口 1 已论证），只能一刀切取严、把前者的认领路一起封掉。

#### 新判据

| 量 | 定义 |
|---|---|
| 目录创建时刻 `ctime` | `os.stat(<爬取根>/<名字>).st_ctime`。探针实测（本机 Windows/NTFS）：「建目录 → 写入文件」后 `st_ctime` **不变**、`st_mtime` 变 ⇒ 它就是创建时刻（`st_birthtime` 本机 Python 不提供） |
| 行时刻 `ts` | `scrape_dn_history.created_at`。SQLite 的 `datetime('now')` 是 **UTC**（探针实测：与 `time.time()` 同轴，差 −0.4s），故解析必须用 `calendar.timegm`；用 `time.mktime` 会整体偏一个时区 |

- **释放行**（真实用户的行）参与判据，当且仅当 **`ts > ctime`** —— 目录是在这行所代表的
  那段持有**期间**建起来的 ⇒ 里面的产物属于这轮持有人。
- **哨兵行**（`user_id = _DN_SENTINEL_UID`）继续**无条件阻断**，但**只对它当年所判的那个化身**：
  **`ts >= ctime`** 才拦。目录若在其后被清掉重建（`ctime` 更新），旧哨兵对新化身失效 ——
  新化身交由释放行的覆盖判据自行裁决。
- **同值（tie）两侧都取严**：释放行用 `>`（同值 ⇒ 不算覆盖）、哨兵行用 `>=`（同值 ⇒ 照拦）。
- 目录**不在根内 / stat 不到** ⇒ 释放行**不过滤**（判据 3 只在目录存在时才用 `hist_keys`，
  这一档对判据 3 **惰性**；保留旧语义也让「释放序查询」在没有目录时仍然成立）；哨兵行照旧拦。
- `created_at` 解析不出来（脏行）⇒ 释放行不算覆盖、哨兵行照旧拦。方向一律 fail-closed。

#### 为什么没把原来的洞重新打开（soundness）

洞的原形态（§0.12）：攻击者 W 曾用过目录名 D 后离开，**别人**后来用 D 建出目录并产出，
W 凭自己那行释放记录认领 ⇒ 读到别人的产物。新判据下 W 的行**不覆盖** D
（他的释放发生在 D 创建**之前**）⇒ 拒。洞仍关着。

放行方向的正确性靠**两条既有不变量**（本次未改一行）：

1. **非 admin 用户的爬取只写进自己的目录名** —— `main._scrape_dn_for:482` 的 `requested_dn`
   只对 `_SCRAPE_CROSS_USER_ROLES`（admin/developer）放行。
2. **同一目录名不可能被两人同时持有** —— 判据 2 + `users.scrape_dn` 唯一索引。

归纳：若我有一行**覆盖** D，说明我最后一段持有跨越了 D 的创建 ⇒ 由 ①，D 里的产物出自
那一段的持有人 = 我（更早的持有段由 ② 排除：接手 D 的前提是 D 当时无主，而「凭曾用名接手」
本身又要求一行覆盖 ⇒ 逐段前推，最早那一段必然跨越创建）。反过来，我的行不覆盖 D
⇒ D 建于我离开之后 ⇒ 内容是别人的 ⇒ 拒。

> 自查记录（如实）：我自己一度用「两段持有」构造反例，推出新判据有洞；回头逐段检查时发现
> 「取新名」必须发生在目录**不存在**的时刻，而那一刻**所有未覆盖的行都被排除**
> ⇒ 反例不成立。走一遍反例再推翻，正是本轮把两条不变量写进文档的原因。

#### 为什么哨兵也要按化身判（且不需要删除任何行）

哨兵有两个来源：迁移时的**歧义名**（跨用户先后无法还原）、扫盘补的**无主目录**。按化身判对
两者都更准，且**不打开洞**：

| 形态 | 新判据下的行为 |
|---|---|
| 歧义名 + 原化身仍在盘上（`ctime` 早于哨兵行） | 哨兵照旧**硬闸** —— 两个持有人都别想认领（他们迁移来的行虽然「覆盖」，但被哨兵一刀切掉）✔ |
| 歧义名 + 目录被清掉重建（`ctime` 晚于哨兵行） | 哨兵失效，但**双方迁移来的行同样不覆盖新化身**（迁移行的 `created_at` 是迁移那一刻，早于任何**新**建的目录）⇒ 谁也认领不了 ✔ |
| 扫盘哨兵（live 的 `alice` / `alice2`） | 原化身仍在 ⇒ 照旧拦，**与今天行为一致、无回归**；若将来目录被清掉重建，该名对新化身的覆盖者开放 ✔ |

⇒ 哨兵行**一律保留**，只加时间维度。**本轮不删任何数据行、不加任何迁移步骤**（纯增量）。

#### 扫盘退役

`database._tombstone_orphan_scrape_dirs` **整段删除**（含 `_migrate_if_needed` 第 6 步的调用）。
理由：它挡的那一档由新判据接住 —— 曾用名持有者的行**不覆盖**无主目录（无主目录建于更早）
⇒ 拒；而**没有**释放行的人本来就越不过判据 3（目录存在、不在 `own_keys`）。
它当年写下的哨兵行保留在表里（见上表：按化身判已足以消除误伤）。旧的一次性标记键
`tombstoned_orphan_scrape_dirs` 保留（历史记录，无用不删）。

⇒ 于是「上线前已改名者」的路**恢复**：他的旧目录建于他改名**之前** ⇒ 他的行覆盖它 ⇒ 放行。

#### 时刻精度：新行必须带亚秒

`note_scrape_dn_release` 改为显式写 `strftime('%Y-%m-%d %H:%M:%f','now')`
（SQLite 的 `%f` **自带秒**，形如 `SS.SSS`，故格式串是 `%H:%M:%f` 而非 `%H:%M:%S.%f`）。
否则秒级截断会把「建完目录、**同一秒内**改名离开」判成 tie ⇒ 取拒 ⇒ **本人**取不回自己的产物
（测试里遍地都是这一档，真实动线上首次爬取后立刻改名也可能撞上）。存量行是老形态（秒级），
两种都解析；秒级行与目录同秒时按 `>` 判为不覆盖（fail-closed，与改动前的行为一致）。

#### 残余风险（如实记录，未消除）

1. **迁移行的 `created_at` 是假时刻** —— 它是**迁移那一刻**，不是真实释放时刻。对「迁移前
   就存在的老目录」，这些行**一律算覆盖**（真时刻不可考）⇒「升级前已删用户的目录 + 存活用户
   曾用名恰同名」那一档**仍有可达性**（扫盘正是为它而建）。**量化**：live 迁移时
   `scrape_dn_history` 为 **0 行**（§0.11 实测），即该档**无实例**；且迁移是一次性的、行集
   固定 ⇒ 不会新增。不假装它不存在，但它当前不可达。
2. **`st_ctime` 的语义依赖平台** —— Windows/NTFS 上是创建时间（本轮实测）；Linux 上是
   **inode 元数据变更**时间（`chmod` / `chown` / 大量拷贝都会推后它）。本部署是 Windows；
   若将来迁 Linux，本判据必须复核。方向性上，`ctime` **偏晚**只会让释放行更容易被判成
   「不覆盖」（fail-closed），不会误放。
3. **秒级存量行与目录同秒** ⇒ 该行的本人可能暂时取不回产物（与改动前一致，非回归）。
4. **行时刻只到毫秒，且是向下截断** —— 写入方用 `strftime('%f')`（毫秒），而目录
   `st_ctime` 是 100ns 分辨率。二者落在**同一毫秒**时，「行时刻」会比 `ctime` 小一丁点
   ⇒ 判成不覆盖（fail-closed）。**可达性**：生产路径上建目录只发生在**爬取**
   （`main.py:660`），写释放行只发生在**改名 / 删号**（另一个请求），两者不可能同毫秒；
   本轮唯一撞上它的是**测试自己**（同一函数里 `os.makedirs()` 紧接着插行），故相关用例改用
   **跨秒 + 睡 50ms** 的确定性构造（见残余风险 5 与第 7 轮 I-1）。不为此放宽判据
   （放宽即动 fail-closed 边界），只记录。
5. **SQLite `now` 与文件系统时钟之间有毫秒级抖动**（自测 300 次采样：偏移 -0.2ms ~ +0.8ms，
   **跨零**）⇒ 严格比较（`ts > ctime` / `ts >= ctime`）在「行与目录相差不到约 1ms」时，方向由
   抖动决定而不是由语义决定。**可达性**：目录由**爬取**建（`main.py:660`），行由**改名 / 删号**
   或**迁移**写 —— 迁移与爬取落在同一毫秒的概率极低；本任务里只有**测试**会构造出这一档，
   首版新用例正因此**假红**过一次（`os.makedirs()` 紧挨着迁移调用），已改成跨秒构造。
   ⚠️ 两侧代价不同：释放行抖到「不覆盖」= 误拒（fail-closed），哨兵抖到「不拦」= 放开
   （fail-open）⇒ 哨兵这侧必须带亚秒（见第 7 轮 I-1）。

#### 对「顺带堵住…未来通道」这句话的更正（自查）

裁定选项里写了「顺带堵住『每周清理后旧释放行认领新目录』这条**未来通道**」——
复核后这句**说重了**，如实更正：那条通道在多数形态下**本来就**被 last-writer-wins
挡住（最后离开的人写下的行既晚于目录、序号也更大 ⇒ 过期的行赢不了比较）。时间维度
**真正**额外挡住的是「行过期、但序号最大」这一形态，即：① 迁移搬来的行（`created_at`
是迁移时刻）；② admin 用 `requested_dn` 代管建出的目录；③ 任何「持有过却没留下行」的
缺口（记录缺失类故障）。它**不会**因此开洞，只是保护面比原话小一档。

#### 测试与变异验证

**退役类的用例改写**（`py/tests/test_scrape_dn_history_migration.py`）：原
`TestOrphanScrapeDirsAreTombstoned`（5 条）整体删除，换成
`TestScanRetirementAndSentinelIncarnations`（5 条），逐条钉住「换判据」而不是「拆保护」：

| 用例 | 钉住什么 |
|---|---|
| `test_migration_no_longer_tombstones_orphan_dirs` | 接线腿（**效果级**）：跑一遍 `_migrate_if_needed`，无主目录不得再被补哨兵 |
| `test_orphan_dir_is_still_not_claimable` | 无主目录**仍然**认领不到（改由判据 3 接住）——退役不放开越权读路径 |
| `test_old_sentinel_still_blocks_on_the_incarnation_it_judged` | 哨兵对它当年所判的化身仍硬闸（含「此刻确实判给我」的夹具前提，防假绿） |
| `test_old_sentinel_does_not_block_a_rebuilt_incarnation` | 目录在其后重建 ⇒ 旧哨兵失效、本人的认领路恢复（本轮正题） |
| `test_sentinel_tie_takes_the_blocking_side` | 哨兵 tie（`ts == ctime`）取拦（纯函数；否则 `>=` 写成 `>` 不会有用例转红） |

**新增的时间维度用例**（`py/tests/test_scrape_ownership.py::TestReleaseMustCoverTheDirectoryIncarnation`，7 条）：

| 用例 | 钉住什么 |
|---|---|
| `test_covering_release_row_can_reclaim` | 放行腿：行晚于目录 ⇒ 认领到（走**生产写入方** + 真实时刻） |
| `test_stale_release_row_cannot_reclaim` | 承重：行早于目录 ⇒ 拒（§0.12 的越权形态） |
| `test_stale_row_of_another_user_does_not_block` | 对照腿：他人**过期**行不再挡我（「放宽」那一侧） |
| `test_rows_are_not_filtered_when_no_directory_exists` | 惰性分支：无目录时不做时间过滤（保住「释放序查询」语义） |
| `test_release_row_is_written_with_subsecond_precision` | 写入方必须带亚秒（断言**格式**，不是 `!= int()`） |
| `test_second_truncated_row_at_the_same_second_is_rejected` | 存量秒级行 + 同秒 ⇒ 拒；含 tie 纯函数对照腿 |
| `test_timestamp_parser_treats_the_column_as_utc` | **定值**钉住时轴（`1970-01-01`→0、`2026-01-01`→1767225600） |

**变异验证**（`temp/_mut_r8.py`，11 条，全部「预期该红确实红了」；跑完按字节还原并核对哈希）：

| 变异 | 预期红（要点） |
|---|---|
| m24 释放行不做时间过滤 | 陈旧腿 + 他人陈旧腿 + 秒级 tie 腿 |
| m25 释放行 tie 改放行（`>`→`>=`） | 秒级 tie 腿（tie 纯函数断言） |
| m26 比较方向写反（`>`→`<`） | 放行腿 + 陈旧腿 + 他人腿 + tie 腿（另连带 9 条既有用例） |
| m27 `_dir_ctime` 恒 None（假装拿不到化身） | 陈旧腿 + 他人腿 + tie 腿 + 重建腿 |
| m28 哨兵 tie 改放行（`>=`→`>`） | 哨兵 tie 腿 |
| m29 写入方退回秒级 | 亚秒格式腿（另连带 5 条既有「改名回来」类用例） |
| m30 哨兵不再硬闸（拿掉 `blocked`） | 既有哨兵硬闸用例 + 老化身哨兵腿 |
| m31 按本地时区解析（`timegm`→`datetime.timestamp()`） | 放行腿 + 他人腿 + **定值腿** + 两条哨兵腿 |
| m32 时间过滤只作用于自己（others 不过滤） | 他人陈旧腿 |
| m33 无目录时也判「不覆盖」 | 惰性分支腿 + 既有迁移顺序用例 |
| m34 扫盘复活（迁移入口给根下每个目录补哨兵） | 退役接线腿 |

⚠️ m31 的方向实测（`temp/_probe_m31.py`）值得记一笔：按本地解析会把每行时刻整体
**提前 8 小时**（本机 UTC+8），方向上仍是 fail-closed ⇒「陈旧行仍拒」那两条腿**照样绿**。
这就是必须有 `test_timestamp_parser_treats_the_column_as_utc` 这条**定值**腿的理由 ——
「哪种错法恰好被别的腿抓到」不能当验收依据。

**回归（第 7 轮处置之前）**：本任务两个测试文件 **100 passed**；全量 `python -m pytest -q` =
**894 passed / 0 failed**。口径：本任务净 +7（退役类 5 换 5、新增 7 条腿），
其余 +10 来自其它在途会话改动的文件（未逐一核对，但与本任务文件无关）。
第 7 轮处置后又补了 3 条腿与 3 条变异（m35–m37），数字见下一节。

#### code-review 第 7 轮处置（2026-09-25）

审查对象 = 本轮工作区改动（`py/auth.py` / `py/database.py` + 两个测试文件）。
结论：**无 Critical**；一条 Important（I-1）已修，Minor 逐条处置如下。

| 编号 | 发现 | 处置 |
|---|---|---|
| I-1 | 迁移写下的**哨兵**行吃表默认值 `datetime('now')`（秒级截断）。同一个截断方向对释放行是 fail-closed，对哨兵（`ts >= ctime`）却是 **fail-open**：同一秒内先建目录、后跑迁移时，这条歧义名会被悄悄放开 —— 此后任何人只要有一条**覆盖**该目录的释放行，就能认领它并读到历史用户留在里面的产物 | **已修**：`database._migrate_scrape_dn_history` 的哨兵 INSERT 改带亚秒（与 `auth.note_scrape_dn_release` 同款，方向改为 fail-closed）。新增效果级用例 `test_migration_sentinel_blocks_the_dir_it_judged`（确定性构造：跨到新一秒 + 睡 50ms）；变异 m35 恰好 1 红 |
| M-1 | 脏行（`ts is None`）的两侧分支都没有用例钉住 —— 往 fail-open 方向改不会有任何用例转红 | **已补**两条纯函数腿：`test_dirty_row_does_not_cover`（释放行取「不覆盖」）与 `test_sentinel_dirty_row_still_blocks`（哨兵取「拦」）；变异 m36 / m37 各恰好 1 红 |
| M-2 | 「名字是否存在」两个谓词口径不一致：`_dir_ctime` 用 `os.path.isdir`，判据 3 对 `listdir` 的**任意**条目都算占用。`root/<D>` 是文件（或指向根外的 junction）时释放行不被过滤 | **不改**：构造不出读路径（该条目不是爬取目录，`makedirs` 会失败；读写两侧另有 `_is_within` 兜底），改它反而会动到存量「同名文件」形态的行为（纯增量原则）。记为**已知不一致** |
| M-3 | 每次名字校验对每个 distinct `(user_id, dn)` 做一次 `isdir` + `stat`，无缓存，而表只增不减 | **不改**：live 仅 8 行、目标规模 20 人，收益低于改动风险。记为**已知成本** |
| M-4 | 注释里「与 `time.time()` 差 -0.4s」口径不对：那是**旧写入方秒级截断**的产物，不是时轴偏移；照它去「校正」反而会引入 -0.4s 的假偏移 | **已改**：按自测（300 次采样，-0.2ms ~ +0.8ms，**跨零**）改写，并点明「别拿它去校正」。审查者给的是 -0.8ms ~ -1.8ms「方向恒为负」—— 那是另一种测法（采样点在 INSERT **之后**，含写入开销），**未采用** |
| M-5 | `AGENTS.md` 同段自相矛盾：「无条件不可被认领」与紧随其后的「按化身生效」冲突 | **已改**：前者加限定语「在该哨兵所判的那个化身仍在时」；并补一条维护不变式「任何写 `scrape_dn_history` 的地方都必须带亚秒」 |
| M-6 | 残余风险表缺哨兵侧（原先只写了释放行的秒级 tie） | **已补**：残余风险 5 |

**M-4 值得单记一笔**：审查者给的那条「方向恒为负」实测，我复现时用的是另一种采样点，
得到的偏移**跨零** —— 两条都不是「判据独立」的交叉验证，只是同一条判据的两种实现。
这也正是必须用**定值**测试（`test_timestamp_parser_treats_the_column_as_utc`）钉住时轴的原因：
「哪种错法恰好被哪条腿抓到」不能当验收依据。

**审查者的附带观察（不在本任务射程）**：其全量跑得 893 passed / 3 failed，3 条红全在
`tests/test_delist_checker.py::TestIndeterminateStatus`，而该文件与其被测对象相对 HEAD **均未修改**，
单独跑、以及与本 diff 的两个文件一起跑都**通过** ⇒ 判为跨文件测试顺序 / 污染（疑似另一在途会话的
产物），不是本 diff 的回归。

**第 7 轮处置后的回归**：本任务两个测试文件 **103 passed**；全量 `python -m pytest -q` =
**907 passed / 0 failed**（260s）。口径：本任务再 +3（I-1 效果腿 + M-1 两条纯函数腿），
其余 +10 仍来自其它在途会话改动的文件。**审查者所见的那 3 条 `test_delist_checker` 红本次未复现**
（上面「附带观察」已记其单跑通过 ⇒ 判为跨文件顺序 / 污染，不是本 diff 的回归）。
变异矩阵扩到 **m24–m37（15 条）**，全跑 `MUTATION_SUMMARY: 0 条未达预期`，
两个源文件按字节还原、哈希一致。
