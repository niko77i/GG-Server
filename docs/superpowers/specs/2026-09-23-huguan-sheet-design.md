...
- **系统 → 表**：账户变更时自动回写那一行；另有一个手动「刷新到看板」按钮做全量刷新。
- **表 → 系统**：手动「从表同步到系统」按钮，先出差异报告，户管确认后才落库。
...
| 5 | 写表时机 | 两者都要：账户变更时自动回写该行 + 手动「刷新到看板」全量 | 2026-09-23 |
...
- 可写列（`writable=True`）：**`A:D` + `F:K`**
- 系统实际回写区间：**`A:D` + `F:G` + `I:K`** —— `H` 列虽然标记可写，但**自动回写永不写入**（§7.2 规则 2），因此它必然落在 `F:G` 与 `I:K` 之间的断口上，不会被任何写入区间覆盖
- 读回范围：**`A:N`**（14 列整段读，未映射列直接忽略）
- 读回**不忽略** `H`（重新分配）：它是归属变更的输入通道，必须读（§7.1）
...
- 可写范围：**`A:J` + `L:M`**（跳过 K）
- 系统实际回写区间：**`A:J` + `M:M`** —— `L` 列虽然标记可写，但**自动回写永不写入**（§7.2 规则 2），故它只由 §7 规则 3① 的通道写入路径单独写（户管在系统 UI 改归属时写该行「换绑情况」列，`py/routes/tt_accounts_routes.py:33`）；`K` 则本就不可写（表中 `field is None`）。两者都落在 `A:J` 与 `M:M` 之间的断口上，不被任何写入区间覆盖
- 读回范围：**`A:M`**
...
- **规则 3（这一列只有两个写点）**：
  ...
  - ② 「从表同步到系统」**成功应用**归属变更后 → 写该行 `重新分配` = `""`（清空）。
...
- 返回文案需区分「已转移至当前用户」与「已从 A 转移至 B」（对照 GG 的 `main.py:4636` / `:4640`，
  旧归属名取法见 `main.py:4609` 的 `old_owner = existing["display_name"] or existing["username"] or "未知"`）。
  **注意**：TT 的 `target_owner == uid` 分支受「逐字节保持原返回文案」硬约束保护，仍是「已转移至当前用户」
  （不含「已从 A」），与 GG 该分支的「已从 A 转移至当前用户」**故意不对称**；只有跨用户分支补「已从 A」。
...
8. `dry_run=false` → 只执行 `confirmed` 里户管勾选的部分，`db.commit()`，清缓存。此路径才允许 `INSERT` 缺失状态（`create_missing=True`），`owner_id` 取该账户当时的 owner。
   ...
   - **`confirmed` 按账户ID 绑定，不按行号。** `confirmed` 形如 `{"create": [账户ID...], "update": [...], "owner": [...]}`。**这是必需的，不是随手选的**：`dry_run=false` 这一步会**重新拉表、重算 diff**（不信任客户端传来的字段值），而户管从看到差异框到点确认之间隔着一段人工时间——正是他会去表里补数据的窗口。若按行号绑定，这期间表里插入/删除/重排一行，他确认的「第 5 行」在重算后的 diff 里已是**另一个账户**，系统会静默改错人、而被挤走的原账户的改动静默丢弃；归属变更按表里为准、权限很大（§12.2），改错人没有第二次机会。按账户ID 绑定后行位移不再影响落库对象。
   - **`not_applied` 消除静默丢弃。** 落库返回里带 `not_applied`：`confirmed` 勾了、但当前 diff 里已找不到该账户ID的项（`[{"account_id":..., "category": "create"|"update"|"owner"}]`）。前端必须把它显示给户管（「以下几行因表已变化未落库」），不能只看 `created/updated/owner_changed` 的计数就报成功。
   - **`dry_run` 是 fail-safe 口径。** 只有**显式布尔 `false`** 才落库；缺省 / `true` / `null` / 字符串 `"false"` 一律按只读处理。`data.get("dry_run", True)` 这种写法下 `"dry_run": null` 会被当成落库（`None` 是假值，反而掉进写分支），属 fail-open，不得使用。判据与「客户端能构造出的畸形 body 只能是 4xx」同源：**能构造出的畸形 body 也绝不能变成一次写库**。
   - **清缓存**（本步骤明文要求）：账户写入后清 `accounts:agents:` 前缀（对照 `main.py:4094`）——**这是本次唯一产生实际失效的清理**（`main.py:3915` 确实写该前缀），测试对它有真实杀伤力。
   - 本次若新建了状态行（任一 diff 项 `pending_status` 非空），再 delete `accounts:statuses:{uid}`。**坦白说这条目前是空操作**：该键全仓库只被 delete、从未被 `set`（`/api/statuses/list` 直查 DB，不缓存），`main.py:6176` / `:6207` 那两处既有 delete 同样是死调用。**仍保留**的理由是**跟随既有约定**——一旦将来给状态下拉加上缓存，两个路径都会自动正确失效。但不要把它当成「已验证的失效逻辑」：钉住它的测试只能证明「调用发生过」，不能证明真的失效了。
   - 清缓存放在**路由层**——纯逻辑的 `huguan_dashboard.py` 不依赖 cache 模块。
...
| 「🔄 刷新到看板」 | `POST /api/huguan/dashboard/push`，全量刷新，出 toast |
| 「⬇️ 从表同步到系统」 | `POST /api/huguan/dashboard/sync` `dry_run=true` → 弹差异确认框 → 确认后 `dry_run=false` |

**差异确认框的强制内容**（「从表同步到系统」的唯一人工关卡，四样都不能省）：
...
4. **落库后的「以下行未落库」提示** —— 读落库返回的 `not_applied`（§8.3 步骤 8），逐条列出账户ID 与类别。**不能只凭 `created/updated/owner_changed` 的计数报成功**：勾了但表已变化、当前 diff 里找不到该账户的行会进 `not_applied` 而非 `errors`，计数里也看不见，漏掉这条提示就等于静默丢单。确认框提交的 `confirmed` 三个键都传**账户ID**（不是行号）。
