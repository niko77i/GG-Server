<template>
  <el-dialog :model-value="visible" @update:model-value="$emit('update:visible', $event)"
    ...
    <el-table :data="rows" size="small" border stripe max-height="400">
      <el-table-column prop="account_id" label="广告账户 ID" width="200" show-overflow-tooltip />
      <el-table-column prop="agent" label="代理" width="100" show-overflow-tooltip />
...
<script setup>
import { ref } from 'vue'
import { ttRechargeApi } from '@/api/tt'
...
async function submit() {
  ...
  saving.value = true
  try {
    const res = await ttRechargeApi.batchSubmit({
      items: records.map(r => ({
    ...
    ElMessage.success(`已提交 ${res.created || 0} 条充值记录`)
