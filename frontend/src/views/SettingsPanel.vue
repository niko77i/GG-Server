    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="账户设置" name="account">
        ...
        <!-- 户管看板配置（子项目 B）。只有户管可见，管理员看到的是上面那张充值表卡片。
             注意：必须落在这个 el-tab-pane 的直接子节点上，不能塞进上面那个
             「管理员专属」的 template（户管不是 admin/developer，那张 template 整体不渲染）。 -->
        ...
        <el-card shadow="never" style="max-width:600px;">
          ...
          <div v-if="regionList.length">
            ...
            >
              ...
              <el-select
                v-if="authStore.canManageAccounts"
                v-model="row._editTz"
                placeholder="选择时区"
                size="small"
                style="flex:1;"
...
// 本卡与上面「📊 充值表配置」互斥（那张对户管整块不渲染），所以不存在主次并列。
// ===========================================================================
const HD_PLATFORM = 'gg'
